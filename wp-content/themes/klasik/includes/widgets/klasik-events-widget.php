...
class Klasik_EventsWidget extends WP_Widget {
  ...
  /** @see WP_Widget::widget */
    function widget($args, $instance) {		
		...
		$onlyfuture 		= isset($instance['onlyfuture']) ? $instance['onlyfuture'] : false;
        ...
        if ( $customclass ) {
            $before_widget = str_replace('class="', 'class="'. $customclass . ' ', $before_widget);
					...
					$background='';
					 ...
					 echo '<div class="klasik-events-widget-wrapper"  style="'.$spacing_left_right.'">';
					...
					$output .='<div class="klasik-events-widget">';
						$output .='<div class="row">';
							...
							if ($wp_query->have_posts()) : 
								...
								if($x%$cols==0){
									...
									//Numeric, with leading zeros-POST-MONTH
									$postnumericmonth  = '';
    ...
    function update($new_instance, $old_instance) {				
		...
		$instance['onlyfuture'] 			= strip_tags($new_instance['onlyfuture']);
    ...
    function form($instance) {
        ...
        ?>
            ...
            <p>
              ...
              <input type="checkbox" name="<?php echo $this->get_field_name('onlyfuture'); ?>" id="<?php echo $this->get_field_id('onlyfuture'); ?>" <?php if($onlyfuture == "on") echo " checked='checked'"; ?> />			
              <label for="<?php echo $this->get_field_id('onlyfuture'); ?>"><?php _e('Show Only Future Post', 'klasik'); ?> </label></p>
